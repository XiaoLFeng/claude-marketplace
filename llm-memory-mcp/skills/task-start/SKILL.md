...
### 多个任务启动（同一 Agent 串行处理）

```javascript
await todo_batch_start({
  codes: ["todo-auth-login", "todo-auth-jwt"]
});
```
...
```markdown
Task Agent 收到任务后，第一步调用：

1. 调用 task-start skill
2. 参数: todo-auth-login
3. 开始编写代码
```

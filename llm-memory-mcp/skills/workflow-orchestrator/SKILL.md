...
```javascript
// 判断是否需要多 Agent
const needMultiAgent = tasks.length > 2 && hasParallelizableTasks(tasks);
```
  ...
  items: [
    {
      ...
      title: "实现登录功能",
      description: "[Task-A] 实现用户登录 API 和前端表单，文件: src/auth/login.ts"
    },
    {
      code: "todo-auth-register",
      title: "实现注册功能",
      description: "[Task-B] 实现用户注册流程，文件: src/auth/register.ts"
    },
    {
      code: "todo-auth-test",
      title: "集成测试",
      description: "[Main] 等待 Task-A/B 完成后执行集成测试"
    }
  ]
});
```
...
📁 文件分配:
  Task-A: src/auth/login.ts, src/auth/jwt.ts
  Task-B: src/auth/register.ts
  Main:   src/auth/index.ts, tests/
...
- `todo_batch_create` - 批量创建带 Agent 标注的任务
- `plan_create` - 创建项目计划
- `todo_list` - 查看任务列表
